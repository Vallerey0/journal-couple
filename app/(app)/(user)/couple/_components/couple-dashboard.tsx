    ...
    <div className="pb-8 space-y-6">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <div className="mt-4 space-y-1">
            <h1 className="text-2xl font-bold tracking-tight text-foreground/90">
              {couple.male_name} <span className="text-pink-500">&</span>{" "}
              {couple.female_name}
            </h1>

            <Badge className="bg-gradient-to-r from-pink-500 to-rose-500 hover:from-pink-600 hover:to-rose-600 border-0 text-white px-3 py-1 rounded-full text-[11px] shadow-lg shadow-pink-500/20">
          ...
          <div className="mt-8 mb-4">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Together since{" "}
              {new Date(couple.relationship_start_date).getFullYear()}
            </p>
          </div>
      ...
      <section className="grid grid-cols-1 gap-4">
        ...
        <div className="space-y-3">
          <h3 className="px-2 text-xs font-bold uppercase tracking-widest text-muted-foreground">
            Partners
          </h3>
      ...
      <section className="grid grid-cols-1 gap-3 pt-4">
        {locked ? (
          <Button
            disabled
            size="lg"
            className="w-full h-14 rounded-2xl bg-muted/50 text-muted-foreground"
          >
          ...
          <Button
            asChild
            size="lg"
            className="w-full h-14 rounded-2xl bg-gradient-to-r from-indigo-500 to-violet-500 hover:from-indigo-600 hover:to-violet-600 shadow-xl shadow-indigo-500/20 text-white font-semibold text-base"
          >
        ...
        <Button
          asChild
          variant="ghost"
          className="w-full h-12 rounded-2xl text-muted-foreground hover:bg-white/5 hover:text-foreground"
        >
...
function PersonCard({
  ...
  return (
    <div
      className={cn(
        "rounded-[24px] p-5 backdrop-blur-md border border-white/10 bg-gradient-to-br",
        gradient,
      )}
    >
      <div className="flex items-start justify-between mb-4">
        ...
        {birth && (showAge || showZodiac) && (
          <Badge
            variant="secondary"
            className="bg-white/20 backdrop-blur-sm border-0 font-normal"
          >
            {showAge && calculateAge(birth) !== null
              ? `${calculateAge(birth)} th`
              : null}
        ...
        {birth && (
          <div className="space-y-1">
            ...
            <p className="text-sm font-medium">
              {new Date(birth).toLocaleDateString("id-ID", {
                day: "numeric",
                month: "short",
              })}
            </p>
